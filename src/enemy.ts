...
export class Enemy extends Entity {
  constructor(model: GLTFShape, transform: Transform) {
    ...
    let endPos = new Vector3(startPos.x, startPos.y, startPos.z - TRAVEL_DISTANCE)
    this.addComponent(
      new utils.MoveTransformComponent(startPos, endPos, TRAVEL_TIME + Math.random() * MAX_TRAVEL_TIME_OFFSET, () => {
    ...
    this.addComponent(
      new OnPointerDown(
        () => {
          ...
          glowingSpaceship.getComponent(Transform).position = this.getComponent(Transform).position
